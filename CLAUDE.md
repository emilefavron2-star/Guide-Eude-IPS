...
23. ⚠️ COMPARER PLUSIEURS ENTITÉS — MODÈLE OBLIGATOIRE « ONGLETS PAR ENTITÉ + ONGLET COMPARER » (référence : `tnc-3` Les grands types de TNC, et `tnc-5` Outils d'évaluation cognitive) : dès qu'un bloc doit présenter **3 entités parallèles ou plus** — types d'une maladie, outils, classes de médicaments, examens, échelles, stades — **ne jamais empiler un grand tableau à N colonnes**. Un tableau large oblige à défiler horizontalement, écrase le texte dans des cellules illisibles, et la comparaison s'y perd. Utiliser le modèle suivant, sans exception.
    ...
    **(f) ⛔ AUCUN CHAPEAU « MODE D'EMPLOI ».** Ne jamais ajouter au-dessus des onglets un texte qui explique *comment se servir de l'encadré* — « trois temps, trois onglets », « cliquez sur ⚖️ Comparer pour… », « ce que ce bloc ajoute », « cinq questions, cinq onglets ». L'utilisateur sait lire une interface : ces phrases ne font que retarder le contenu, et elles vieillissent mal dès qu'un onglet est ajouté ou renommé. Les libellés des onglets doivent suffire — s'ils ne suffisent pas, c'est le libellé qu'il faut corriger. **Seule exception** : la `key-fact` de l'onglet ⚖️ Comparer qui dit **quel axe discrimine** (règle #23b) — celle-là est du raisonnement clinique, pas du mode d'emploi. Test pour trancher : si la phrase resterait vraie en changeant de sujet clinique, c'est un mode d'emploi, donc à supprimer.
...
25. ⚠️ TOUTE ÉCHELLE, TOUT SCORE, TOUT OUTIL DE DÉPISTAGE SE PRÉSENTE EN TROIS TEMPS — BUT, CRITÈRES, INTERPRÉTATION CHIFFRÉE : nommer une échelle sans dire ce qu'on en fait est inutilisable. Chaque fois qu'une échelle est intégrée au guide (NICE, CNST, DIET, PANSS, UPDRS, DAS28, CURB-65, Wells, CAM, Fried, Rome IV, Bethesda, Balthazar, Partin, Mayo, SES-CD, Harvey-Bradshaw, Truelove-Witts…), les trois éléments suivants sont obligatoires, dans cet ordre :

    **(a) LE BUT — une phrase.** Qu'est-ce que l'échelle prétend mesurer, chez qui, et à quel moment du raisonnement elle sert (dépister ? confirmer ? stratifier la sévérité ? prédire une complication ? suivre une réponse au traitement ?). Une échelle de **dépistage** et une échelle de **sévérité** ne s'utilisent pas au même moment et n'autorisent pas les mêmes conclusions — le dire explicitement.

    **(b) LES CRITÈRES — tous, et chiffrés.** La liste complète des items, chacun avec son seuil (règle #22). Si l'échelle distingue des **critères majeurs et mineurs**, ou **primaires et secondaires**, la distinction doit apparaître visuellement — deux grilles, ou deux colonnes, jamais une liste indifférenciée. Utiliser une `criteria-list` (règle #16d) ou un tableau à deux colonnes.
